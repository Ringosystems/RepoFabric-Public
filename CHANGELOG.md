...
- **First public release, single-image distribution**: RepoFabric is published as one container image on Docker Hub (`ringosystems/repofabric`) and GitHub Container Registry, MIT licensed. The Sandbox trial and both production flavours run the same image and differ only at runtime, through one environment variable and the surrounding compose stack. A publish workflow builds once, scans for fixable HIGH and CRITICAL CVEs, then pushes to both registries.
